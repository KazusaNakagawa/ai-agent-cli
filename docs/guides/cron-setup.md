...
This is an **alternative** scheduling setup. The **currently active** approach is manual `./bin/workflow.sh run daily` — see [launchd-setup.md](launchd-setup.md#manual-execution-active). launchd is documented there for always-awake / AC setups; use **at most one** scheduler (cron, launchd, or manual). `bin/workflow.sh` (root-level wrapper) sources `.env` and delegates to `apps/python/bin/workflow.sh`, so API credentials are available in non-interactive shells.
...
0 5 * * * cd /Users/YOUR_USERNAME/work/ai-agent && ./bin/workflow.sh run daily >> /Users/YOUR_USERNAME/work/ai-agent/log/cron-run.log 2>&1
```

| Part | Why |
|---|---|
| `PATH=...` line | cron's default PATH lacks `/opt/homebrew/bin`, where the `claude` CLI lives |
| `cd <repo>` | `bin/workflow.sh` resolves paths from its own location, but `cd` keeps relative output paths predictable |
...
Note that `apps/python/bin/workflow.sh` re-execs itself under `caffeinate -ims`, which prevents sleep *during* the run. It cannot wake a Mac that is already asleep when cron fires — that is what `pmset` covers.
...
```bash
cd ~/work/ai-agent && ./bin/workflow.sh run daily
```
