...
> **実装後の追記（#472）**: 以下の「現状（調査結果）」は移植前のスナップショットで、当時の判断根拠として残している。
> briefing は #461、weekly は #463 で移植済み。`bin/run.sh` が担っていた「ブリーフィング → 週次リキャップ」の合成は
> `daily` ワークフロー（`src/workflow/definitions/daily.py`）になり、スクリプト自体は廃止された。
> 現在の日次の入口は `bin/workflow.sh run daily`。
